...
# calculate the percentage from max and current durability then apply to scoreboard
# This needs to be reworked, as doenst give proper output.
# max / current * 100 = N as %
execute at @p[nbt={SelectedItem:{tag:{durability:{}}}}] store result score @p durabilityPer run data get entity @p SelectedItem.tag.durability.max_durability
execute at @p[nbt={SelectedItem:{tag:{durability:{}}}}] run scoreboard players operation @p durabilityPer /= int util
